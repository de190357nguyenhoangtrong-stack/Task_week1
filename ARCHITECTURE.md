...
### 1.5 Thư viện cấm dùng nếu chưa có phê duyệt CTO
...
Mọi project landing page phải tuân theo cấu trúc này. Không thêm thư mục gốc nếu không có lý do kỹ thuật rõ ràng.
...
| Biến môi trường public | Prefix `NEXT_PUBLIC_` | `NEXT_PUBLIC_GTM_ID` | `GTM_ID` thiếu prefix |
...
// 1. Định nghĩa Props với TypeScript interface
// KHÔNG dùng type nếu có thể extend
...
| Không inline style | Chỉ dùng Tailwind. Nếu cần dynamic value thì dùng CSS variable qua `style={{}}` hạn chế. |
...
### 4.3 Tracking events chuẩn

Mọi CTA phải có tracking event.
...
# Form / Backend
FORM_SUBMIT_WEBHOOK_URL= # Webhook nhận lead
FORM_SUBMIT_SECRET=      # Secret key verify webhook
...
| 7 | CTA Events | Click CTA thấy event `cta_click` trong GA4 DebugView | Marketing lead |
| 8 | Form Events | Submit form thấy `form_submit_success` trong GA4 DebugView | Marketing lead |
...
Nếu không có blocker, dòng 5 ghi:
